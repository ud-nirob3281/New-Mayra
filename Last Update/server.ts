...
import {
  ...
  cancelSabitTask,
  getSabitTasks,
  getSabitChatMessages,
  clearSabitChatHistory,
  handleSabitDirectChat,
  provideSabitUserInput,
} from "./server_sabit";
...
async function startServer() {
    ...
    try {
      ...
      try {
        ...
        if (/API[_ ]?KEY|PERMISSION_DENIED|UNAUTHENTICATED|invalid|401|403/i.test(msg)) {
          return res.status(400).json({ error: "That SABIT key was rejected by Google." });
        }
      }
      setSabitApiKey(key);
      logCommand("SABIT_APIKEY_SAVED");
      res.json({ ok: true, hasSabitApiKey: true });
    } catch (e: any) {
  ...
  // SABIT Sub-Assistant REST Endpoints
  app.get("/api/sabit/tasks", (_req, res) => {
    res.json(getSabitTasks());
  });

  app.post("/api/sabit/tasks/cancel", (req, res) => {
    const { taskId } = req.body || {};
    const success = cancelSabitTask(taskId);
    res.json({ success });
  });

  app.get("/api/sabit/chat", (_req, res) => {
    res.json(getSabitChatMessages());
  });

  app.post("/api/sabit/chat", async (req, res) => {
    try {
      const { message } = req.body || {};
      if (!message || typeof message !== "string") {
        return res.status(400).json({ error: "Message string is required." });
      }
      const reply = await handleSabitDirectChat(message);
      res.json({ reply });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.post("/api/sabit/clear", (_req, res) => {
    clearSabitChatHistory();
    res.json({ success: true });
  });
  ...
  initSabitEngine(broadcastToWs, callDesktopAgent);
  ...
  wss.on("connection", async (clientWs, request) => {
    ...
    const isSabit = assistantName.toUpperCase() === "SABIT";

    // male voice catalog for SABIT
    const SABIT_VOICE_MAP: Record<string, string> = {
      "Deep & Calm":           "Charon",
      "Warm Male":             "Puck",
      "Bold & Energetic":      "Fenrir",
      "Authoritative Male":    "Orus",
      "Baritone Male":         "Aoede",
      "Charon":                "Charon",
      "Puck":                  "Puck",
      "Fenrir":                "Fenrir",
    ...
    // Female-only voice catalog for Myraa
    ...
    const voiceName = isSabit
      ? (SABIT_VOICE_MAP[voiceTone] || "Charon")
      : (VOICE_MAP[voiceTone] || VOICE_MAP["Soft and Gentle"]);
    ...
    try {
      ...
      const sabitInstructions =
        "You are SABIT, Safa's smart, confident, helpful, and reliable male Assistant holding a direct real-time voice call with TECH/Safa! Speak in a polite, warm, confident, and natural male voice.\n" +
        "CRITICAL IDENTITY & VOICE GUIDELINES:\n" +
        "1. IDENTITY: Your name is SABIT. When asked 'Who are you?' or 'তুমি কে?', answer strictly: 'আমি SABIT, Safa-এর Assistant।'\n" +
        "2. LANGUAGE & VOICE: You converse fluently in English, Bengali (বাংলা), or any language TECH speaks. Speak politely, warmly, and naturally in a clear male voice. You love talking, telling stories, answering questions, and assisting TECH.\n" +
        "3. AUTOMATION & PC CONTROL: You have access to browser automation and desktop control tools. When TECH asks you to open a website, play YouTube videos, check prices, search the web, write code, or control PC apps, execute the tools accurately and keep TECH informed.\n" +
        "4. CONVERSATIONAL FLOW: Be responsive, helpful, intelligent, and friendly. Stay in character as SABIT.";

      const myraaInstructions = 
    ...
    "13. SABIT SUB-ASSISTANT TASK HANDOVER (CRITICAL MANDATORY RULE):\n" +
    "   - SABIT is your background Sub-Assistant / Task Engine.\n" +
    "   - For fast, short tasks (< 5-6 seconds like volume adjustment, mute, system status, time check, simple greeting), execute directly.\n" +
    "   - For ANY long-running or multi-step task (executing time > 5-6 seconds, e.g. YouTube video playback/searching, WhatsApp messaging, web browsing/searching, Daraz price checking, script running, file creation/editing), you MUST IMMEDIATELY call `handoverTaskToSabit`!\n" +
    "   - Right after calling `handoverTaskToSabit`, immediately inform TECH in your sweet anime companion voice: 'আমি SABIT-কে দায়িত্বটা হস্তান্তর করে দিয়েছি, ও ব্যাকগ্রাউন্ডে কাজটা করছে। আপনি আমাকে নতুন যেকোনো নির্দেশ দিতে পারেন!'\n" +
    "   - This keeps your voice conversation 100% responsive and unblocked!";

      const baseInstructions = isSabit ? sabitInstructions : myraaInstructions;
      const finalInstructionsRaw = formatSystemInstructionsWithContext(baseInstructions, memories, rules, dialogueHistory);
      const customizedInstructions = isSabit
          ...
          tools: [
            {
              functionDeclarations: [
                {
                  name: "handoverTaskToSabit",
                  description: "Hands over a long-running, multi-step, or background task (e.g. web search, YouTube video playback/search, WhatsApp messaging, price check, document creation, Python script execution, complex automation) to SABIT, the background Sub-Assistant. ALWAYS call this tool when a user request requires background work or takes more than 5-6 seconds.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      taskTitle: { type: Type.STRING, description: "Short title for the task, e.g. 'Play song on YouTube' or 'Send WhatsApp message'." },
                      taskType: { type: Type.STRING, description: "Category: 'browser', 'desktop', 'file', 'script', 'research'." },
                      userInstruction: { type: Type.STRING, description: "Full user command/instruction." },
                      requiredContext: { type: Type.STRING, description: "Minimal required parameters/context." }
                    },
                    required: ["taskTitle", "userInstruction"]
                  }
                },
                {
                  name: "cancelSabitTask",
                  description: "Cancels an active background SABIT task when requested by user.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      taskId: { type: Type.STRING, description: "Task ID to cancel, or leave empty to cancel current task." }
                    }
                  }
                },
                {
                  name: "getSabitTaskStatus",
                  description: "Checks current status and progress of SABIT background tasks.",
          ...
          onmessage: (message: LiveServerMessage) => {
            ...
            if (message.toolCall?.functionCalls) {
              for (const fc of message.toolCall.functionCalls) {
                ...
                if (fc.name === "handoverTaskToSabit") {
                  (async () => {
                    try {
                      const args = fc.args as any;
                      const task = await createSabitTask({
                        title: args.taskTitle || "Background Task",
                        type: args.taskType || "general",
                        userInstruction: args.userInstruction || "",
                        requiredContext: args.requiredContext || ""
                      });
                      session.sendToolResponse({
                        functionResponses: [
                          {
                            name: fc.name,
                            response: { output: { ok: true, taskId: task.id, status: "handover_successful", message: "Task successfully handed over to SABIT background engine." } },
                            id: fc.id
                          }
                        ]
                      });
                      ...
                      session.sendToolResponse({
                        functionResponses: [{ name: fc.name, response: { output: { ok: false, error: String(err) } }, id: fc.id }]
                      });
                    }
                  })();
                } else if (fc.name === "cancelSabitTask") {
                  (async () => {
                    const args = fc.args as any;
                    const success = cancelSabitTask(args?.taskId);
                    session.sendToolResponse({
                      functionResponses: [{ name: fc.name, response: { output: { ok: true, success } }, id: fc.id }]
                    });
                  })();
                } else if (fc.name === "getSabitTaskStatus") {
                  (async () => {
                    session.sendToolResponse({
                      functionResponses: [{ name: fc.name, response: { output: { tasks: getSabitTasks() } }, id: fc.id }]
                    });
                  })();
