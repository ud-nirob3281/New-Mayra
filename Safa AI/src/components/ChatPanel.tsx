...
import { Send, Trash2, X, MessageSquare, Copy, Check, Mic, Volume2, Brain, Cpu, Sparkles } from "lucide-react";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";

/**
 * ChatPanel — MYRAA's real messaging & transcript interface.
 * Displays unified conversation history (User Voice, User Text, Safa Voice, Safa Text, Safa Thinking, Tool Results)
...
export interface ChatMessage {
  ...
  text: string;
  messageType?: "user_voice" | "user_text" | "safa_thinking" | "safa_voice" | "safa_text" | "tool_result" | string;
        ...
        {messages.length === 0 ? (
          ...
          <AnimatePresence initial={false}>
            {messages.map((msg) => {
              ...
              const isThinking = msg.messageType === "safa_thinking";
              const isTool = msg.messageType === "tool_result";
              const isVoice = msg.messageType === "user_voice" || msg.messageType === "safa_voice";
              ...
              return (
                ...
                >
                  {/* Badge Label */}
                  <div className="flex items-center gap-1.5 mb-1 px-1 text-[10px] font-mono tracking-wider uppercase text-slate-400">
                    {msg.messageType === "user_voice" && (
                      <span className="flex items-center gap-1 text-blue-400">
                        <Mic size={11} /> Voice Input
                      </span>
                    )}
                    {msg.messageType === "user_text" && (
                      <span className="flex items-center gap-1 text-blue-300">
                        <MessageSquare size={11} /> Text Input
                      </span>
                    )}
                    {msg.messageType === "safa_voice" && (
                      <span className="flex items-center gap-1 text-pink-300">
                        <Volume2 size={11} /> {assistantName} Voice
                      </span>
                    )}
                    {msg.messageType === "safa_text" && (
                      <span className="flex items-center gap-1 text-cyan-300">
                        <Sparkles size={11} /> {assistantName} Text
                      </span>
                    )}
                    {msg.messageType === "safa_thinking" && (
                      <span className="flex items-center gap-1 text-amber-300">
                        <Brain size={11} /> {assistantName} Thinking
                      </span>
                    )}
                    {msg.messageType === "tool_result" && (
                      <span className="flex items-center gap-1 text-purple-300">
                        <Cpu size={11} /> Tool Output
                      </span>
                    )}
                  </div>
                  ...
                  <div
                    className={`relative max-w-full p-3 rounded-lg text-sm leading-relaxed ${
                      isUser
                        ...
                        : isThinking
                        ? "bg-amber-500/10 border border-amber-500/30 text-amber-100 italic"
                        : isTool
                        ? "bg-purple-950/40 border border-purple-500/30 font-mono text-xs"
