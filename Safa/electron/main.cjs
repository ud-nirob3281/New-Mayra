...
// A leftover server from a previous app run can still be listening on port
// 3000. When that happens the NEW backend dies with EADDRINUSE and the app
// silently keeps talking to the OLD process running OLD code — which is why
// session/memory fixes appeared to "never work". Since we hold the
// single-instance lock, anything on port 3000 at boot is a zombie: kill it.
  ...
  return new Promise((resolve) => {
    ...
    netstat.on('close', () => {
      ...
      for (const line of out.split('\n')) {
        if (line.includes(`:${SERVER_PORT} `) && line.trim().startsWith('TCP') && /LISTENING/i.test(line)) {
      ...
      for (const pid of pids) {
        console.log(`[Main] Killing stale port-${SERVER_PORT} process (PID ${pid}) from a previous run…`);
        const kill = spawn('taskkill', ['/PID', pid, '/T', '/F'], { windowsHide: true });
        const done = () => { if (--pending <= 0) resolve(); };
        kill.on('error', done);
        kill.on('close', done);
      }
      // Safety net so boot never stalls on taskkill
