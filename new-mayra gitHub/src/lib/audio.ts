...
export class MyraaAudioSession {
   ...
   */
  private reconnect() {
    // Tear down old WS + audio nodes without flipping isActivated off
    this.cleanupResources();
    // Wait a beat, then re-establish the connection
    setTimeout(() => {
      if (this.currentState === "disconnected") return; // user gave up
      try {
        this.isActivated = false; // allow connect() to run again
